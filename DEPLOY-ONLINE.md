...
3. **Root Directory** → **ha beddelin** (ka tag `/` — repo root). Root-ka waxaa ku jira `Dockerfile` + `railway.toml` oo backend-ka build gara.
4. Haddii aad hore ugu dejisay Root = `backend`, dib ugu celi **/** ama tirtir Root Directory.
5. **Settings** → **Build** → waa inuu muujiyaa **Dockerfile** (ma aha Railpack auto-detect oo keliya)
...
| Qalad | Xal |
|-------|-----|
| Railway build fail / Railpack error | Hubi root-ka repo `Dockerfile` + `railway.toml` jiraan; Root Directory = `/` (empty) |
